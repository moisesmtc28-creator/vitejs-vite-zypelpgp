...
export default function App() {
  ...
  const [treinoOrigemId, setTreinoOrigemId] = useState("");
  ...
  async function criarTreinoAPartirTreinoExistente() {
    if (!usuario) return;

    if (!alunoSelecionado) {
      alert("Selecione o aluno que vai receber a ficha.");
      return;
    }

    if (!treinoOrigemId) {
      alert("Selecione um treino existente para reaproveitar.");
      return;
    }

    const aluno = alunos.find((a) => a.id === alunoSelecionado);
    const treinoOrigem = treinos.find((t) => t.id === treinoOrigemId);

    if (!aluno || !treinoOrigem) {
      alert("Aluno ou treino de origem não encontrado.");
      return;
    }

    const nomeFicha =
      prompt("Nome da nova ficha para o aluno:", nomeTreino || treinoOrigem.nome) ||
      nomeTreino ||
      treinoOrigem.nome;

    const ref = await addDoc(collection(db, "treinos"), {
      nome: nomeFicha,
      dataTreino: dataTreino || new Date().toISOString().slice(0, 10),
      dataCriacao: new Date().toISOString(),
      versaoFicha: 2,
      alunoId: aluno.id,
      alunoNome: aluno.nome,
      alunoEmail: aluno.email,
      professorEmail: usuario.email,
      exercicios: (treinoOrigem.exercicios || []).map(limparExercicioParaModelo),
      mensagens: [],
      origemTreinoId: treinoOrigem.id,
      origemTreinoNome: treinoOrigem.nome,
      criadoEm: new Date(),
      atualizadoEm: new Date().toISOString(),
    });
    ...
    alert("Treino reaproveitado com sucesso. Agora você pode editar a nova ficha.");
    carregarTudo();
  }
      ...
      {perfil?.tipo === "professor" && (
        <div style={styles.grid2}>
          ...
          <Card>
            ...
            <div style={styles.reaproveitarBox}>
              <h3 style={{ marginTop: 0 }}>Reaproveitar treino</h3>
              <p style={{ marginTop: 0 }}>
                Você pode criar uma nova ficha usando um modelo salvo ou copiando um
                treino existente de qualquer aluno deste professor.
              </p>

              <label style={styles.label}>Usar modelo salvo</label>
              <select
                style={styles.input}
              ...
              <label style={styles.label}>Copiar treino existente</label>
              <select
                style={styles.input}
                value={treinoOrigemId}
                onChange={(e) => setTreinoOrigemId(e.target.value)}
              >
                <option value="">Selecione um treino já criado</option>
                {treinos.map((treinoModelo) => (
                  <option key={treinoModelo.id} value={treinoModelo.id}>
                    {treinoModelo.nome} - {treinoModelo.alunoNome || "Aluno"} - {treinoModelo.exercicios?.length || 0} exercícios
                  </option>
                ))}
              </select>

              <button style={styles.primary} onClick={criarTreinoAPartirTreinoExistente}>
                Copiar treino para o aluno selecionado
              </button>
            </div>
          </Card>
        </div>
                      ...
                      <div
                        ...
                        style={{
                          ...
                          background: perfil?.tipo === "aluno"
                            ...
                            : (ex.finalizado ? "#123524" : "#111827"),
                          color: "#ffffff",
                          border: perfil?.tipo === "aluno"
                            ? (ex.finalizado ? "3px solid #22c55e" : "3px solid #334155")
                            : (ex.finalizado ? "2px solid #22c55e" : "2px solid #334155"),
                        }}
                      >
...
const styles: any = {
  ...
  label: { fontWeight: "bold", display: "block", color: "inherit" },
  primary: {
    padding: "10px 14px",
    margin: 5,
    borderRadius: 10,
